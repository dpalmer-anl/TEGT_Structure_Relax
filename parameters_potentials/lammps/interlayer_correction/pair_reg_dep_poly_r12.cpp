...
double PairRegDepPoly::init_one(int i, int j)
{
  ...
  if (offset_flag && (cut_global > 0.0)) {
    ...
    offset[i][j] = -(p.A6*pow(p.z0/cut_global,6) + p.A8*pow(p.z0/cut_global,8)
	     + p.A10*pow(p.z0/cut_global,10)+p.A12*pow(p.z0/cut_global,12));
  } else offset[i][j] = 0.0;
...
void PairRegDepPoly::read_file(char *filename)
{
  ...
  params = nullptr;
  nparams = maxparam = 0;
  int NPARAMS_PER_LINE = 14; // open file on proc 0
  ...
  if (comm->me == 0) {
	...
	params[nparams].A12      = values.next_double();
      ...
      params[nparams].z012 = pow(params[nparams].z0,12);
...
void PairRegDepPoly::calc_FvdW(int eflag, int /* vflag */)
{
  ...
  double r2inv,r6inv,r8inv,r10inv,r12inv,r14inv;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	r14inv = r12inv*r2inv; //r^(-14) (only used for derivatives)
        ...
        sumP = -1*(p.A6*p.z06*r6inv + p.A8*p.z08*r8inv + p.A10*p.z010*r10inv + p.A12*p.z012*r12inv); // -C6*(r/z0)^-6 - C8*(r/z0)^-8 - C10*(r/z0)^-10 - C12*(r/z0)^-12
        ...
        fpair = -(6.0*p.A6*p.z06*r8inv + 8.0*p.A8*p.z08*r10inv + 10.0*p.A10*p.z010*r12inv + 12*p.A12*p.z012*r14inv)*sumCff;
...
double PairRegDepPoly::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
                         ...
                         double &fforce)
{
  double r,r2inv,r6inv,r8inv,r10inv,r12inv,r14inv,forcelj,philj;
  ...
  Vkc = -(p.z06*r6inv + p.z08*r8inv + p.z010*r10inv+ p.z012*r12inv);
  // derivatives
